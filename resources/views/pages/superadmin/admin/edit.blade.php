...
@section('content')
    ...
    <div class="mb-4 shadow card">
        ...
        <div class="card-body">
            <form action="{{ route('admin.admin.update', Crypt::encrypt($admin->id)) }}" method="POST"
                ...
                <div class="form-group">
                    ...
                    @error('name') is-invalid @enderror"
                        id="name" name="name" value="{{ old('name', $admin->user->name) }}" required minlength="3">
                    @error('name')
                    ...
                    @error('title') is-invalid @enderror" id="faculty_id"
                        ...
                        @foreach ($faculties as $faculty)
                            <option value="{{ $faculty->id }}"
                                {{ $faculty->id == old('faculty_id', $admin->faculty->id) ? 'selected' : '' }}>
                                {{ $faculty->name }}
                            </option>
                        @endforeach
                    </select>
